...
def query_db(query, args=(), one=False):
    ...
    try:
        con = sqlite3.connect(f'file:{DB_PATH}?mode=ro', uri=True) # Read-only connection
...
def get_history():
    ...
    if not sensor or not metric: return jsonify({"error": "Missing 'sensor' and 'metric' parameters"}), 400
    ...
    if results is None: return jsonify({"error": "Database query failed"}), 500
...
def set_config():
    ...
    if not new_config: return jsonify({"error": "Invalid JSON payload"}), 400
    with config_lock:
