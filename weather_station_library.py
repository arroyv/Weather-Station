...
class RainGaugeSensor:
    ...
    def __init__(self, name, feed_name, gpio_pin, mm_per_tip, **kwargs):
        ...
        # gpiod objects
        # On Pi 5, the main GPIO controller is chip 4. On older Pis it might be 0.
        self.chip = gpiod.Chip('gpiochip4') 
        self.gpio_line = self.chip.get_line(self.gpio_pin)
        
        # Configure the line for input with a pull-up resistor and to detect falling edges.
        self.gpio_line.request(
            consumer="weather-station-rain",
            type=gpiod.LINE_REQ_EV_FALLING_EDGE,
            flags=gpiod.LINE_REQ_FLAG_PULL_UP
        )

    def _event_monitor_thread(self):
        """A dedicated thread to wait for GPIO events."""
    ...
    def start(self):
        ...
        # Start the thread that listens for tips
    ...
    def stop(self):
        ...
        self._stop_event.set()
        # Release the GPIO line so other programs can use it
