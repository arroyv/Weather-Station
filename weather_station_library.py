# weather_station_library.py (Corrected for all bugs)
    ...
    def process(self, data_packet):
        ...
        for sensor_name, metrics in data_packet.items():
            for metric_name, value in metrics.items():
                try:
                    # Find the explicit feed_key from the config
                    if sensor_name == 'rain':
                        feed_key = self.config['rain_gauge'].get('feed_key')
                    else:
                        # Find the sensor's address to look up its config
                    ...
                    if not feed_key:
                        ...
                        feed_key = f"{sensor_name}-{metric_name}"
...
# ... (SQLiteHandler and DataCacheHandler are unchanged) ...
class SQLiteHandler:
    def __init__(self, db_path): self.db_path, self.conn = db_path, None
    def start(self):
        self.conn = sqlite3.connect(self.db_path, check_same_thread=False); self.conn.cursor().execute('''CREATE TABLE IF NOT EXISTS readings (timestamp TEXT, sensor TEXT, metric TEXT, value REAL, PRIMARY KEY (timestamp, sensor, metric))'''); self.conn.commit()
    ...
    def process(self, data_packet):
        ...
        try:
            cur = self.conn.cursor(); cur.executemany("INSERT INTO readings VALUES (?, ?, ?, ?)", insert_data); self.conn.commit()
            print(f"  [SQLiteHandler]   → Wrote {len(insert_data)} records.")
        except sqlite3.Error as e: print(f"  [SQLiteHandler]   → ERROR: {e}")
class DataCacheHandler:
    def __init__(self): self._cache, self._lock = {}, Lock()
    def process(self, data_packet):
        with self._lock: self._cache['timestamp'] = datetime.datetime.now(datetime.timezone.utc).isoformat(); self._cache['sensors'] = data_packet
        print(f"  [DataCacheHandler]   → Latest data cached.")
    def get_latest_data(self):
        with self._lock: return self._cache.copy()

# ============================================================================
# WEATHERSTATION CLASS (Unchanged)
# ============================================================================
class WeatherStation:
    ...
    def load_config(self):
        print("  [Config] Loading configuration..."); with open(self.config_path, 'r') as f: self.config = json.load(f)
        self.last_config_mtime = os.path.getmtime(self.config_path); return self.config
    def start_all(self):
        print("\n--- Starting All Services ---"); [h.start() for h in self.handlers]; [s.start() for s in self.sensors.values()]
        Thread(target=self._data_dispatcher_loop, daemon=True).start(); Thread(target=self._config_watcher_loop, daemon=True).start()
    def stop_all(self):
        self._stop_event.set(); [s.stop() for s in self.sensors.values()]; [h.stop() for h in self.handlers]
    def _data_dispatcher_loop(self):
        # This loop is now correct and doesn't need changes
        upload_rate = self.config.get('upload_rate', 300); self._stop_event.wait(10)
        while not self._stop_event.is_set():
            ...
            upload_rate = self.config.get('upload_rate', 300); self._stop_event.wait(upload_rate)
    def _config_watcher_loop(self):
        # This is also correct and doesn't need changes
        while not self._stop_event.is_set():
            try:
                mtime = os.path.getmtime(self.config_path)
                if mtime > self.last_config_mtime: print("\n  [ConfigWatcher] Detected config file change. Reloading..."); self.reload_config()
...
# ... (RainGaugeSensor class is unchanged and still correct) ...
class RainGaugeSensor:
    ...
    def _daily_reset_thread(self):
        while not self._stop_event.is_set():
            now = datetime.datetime.now(); next_midnight = (now + datetime.timedelta(days=1)).replace(hour=0, minute=0, second=1)
    ...
    def get_latest_readings(self):
        with self._count_lock: return {self.metric: round(self.tip_count * self.mm_per_tip, 2)}